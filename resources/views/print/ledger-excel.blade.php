...
$receivedFrom = $item->journal->cashIn->received_from ?? '';
$description = $item->journal->cashIn->description ?? '';
return $description . ' | Diterima Dari ' . $receivedFrom;
} else if ($item->journal->transaction_type === "CashOut") {
$paidTo = $item->journal->cashOut->paid_to ?? '';
$description = $item->journal->cashOut->description ?? '';
return $description . ' | Dibayar Kepada ' . $paidTo;
    ...
    <thead>
        <tr>
            <td colspan="7" style="text-align: center; font-weight: bold; font-size: 16px;">
               Buku Besar {{ $chartOfAccount?->name ?? 'Silahkan Pilih Akun' }} Periode {{$dateFrom}} - {{$dateTo}}
            </td>
        </tr>
    </thead>
</table>
<table class="items-table">
    <thead>
        <tr>
            <th colspan="7">
                {{ $chartOfAccount?->name ?? 'Silahkan Pilih Akun' }} Periode {{$dateFrom}} - {{$dateTo}}
            </th>
        </tr>
        <tr>
            <th>NO</th>
    ...
    <tbody>
        <tr>
            ...
            <td></td>
            <td></td>
            <td class="text-end">{{$startBalance}}</td>
        </tr>
    ...
    <tfoot>
        <tr>
            ...
            <td class="text-end">{{$ledgers?->sum('debit') }}</td>
            <td class="text-end">{{$ledgers?->sum('credit')}}</td>
