...
<body>
    ...
    <table>
        ...
        <tbody>
            ...
            @endforeach
            <tr>
                <td>{{$cashIn->details->count() + 1}}.</td>
                <td>Penjualan {{$cashIn->sale->no}}/{{$cashIn->sale->cr_db}}/{{$cashIn->sale->company_sale_code}}/{{$cashIn->sale->month}}/{{$cashIn->sale->year}}</td>
                <td>IDR</td>
                <td></td>
                <td class="text-right">{{rupiah($cashIn->sale->total_amount)}}</td>
            </tr>
            <tr class="total">
