...
import { AssetType } from "@/Pages/Asset/@types/asset-type"
...
export default function SearchAsset({ onChange, value, clearable }: SearchAssetProps) {
    ...
    const prevValueRef = useRef<AssetType | undefined>(value)
    ...
    const handleCodeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        setAsset({ ...asset, code: e.target.value })
    ...
    const searchByCode = async (code: string) => {
        ...
        try {
            const { data } = await axios.get(`/admin/asset/search-by-code?search=${code}`)
            ...
            setError("Failed to search asset")
            console.error(err)
        } finally {
            setIsLoading(false)
        }
    ...
    const searchByName = async (name: string) => {
        ...
        try {
            const { data } = await axios.get(`/admin/asset/search-by-name?search=${name}`)
    ...
    const handleCodeKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === "Enter") {
            searchByCode(asset.code!)
        }
    }

    const handleNameKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === "Enter") {
            searchByName(asset.name!)
        }
    }
    ...
    const handleSearch = (query: string) => {
        ...
        axios.get(`/admin/asset/browse?search=${query}`)
            .then((response) => {
        ...
        <div className="space-y-2 ">
            <div className="relative">
                ...
                <div className="flex">
                    ...
                    <div>
                        <Button className="rounded-none border " onClick={handleSearchButtonClick} disabled={isLoading}>
                ...
                {/* {asset.id && !isLoading && !error && (
                <div className="text-sm text-muted-foreground">
                    Asset found: {asset.name}
                </div>
            )} */}
            </div>
        </div>
