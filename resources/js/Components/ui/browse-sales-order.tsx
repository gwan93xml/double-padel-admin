...
interface BrowseSalesOrderProps extends React.ComponentPropsWithoutRef<typeof SelectPrimitive.Root> {
...
export default function BrowseSalesOrder({ value, label, remoteConfig, onChange, initialSelected }: BrowseSalesOrderProps) {
        ...
        <>
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                <DialogContent className="md:max-w-[70%] ">
                    <DialogHeader>
                        <DialogTitle>Data Sales Order</DialogTitle>
                        <DialogDescription>
                            Silahkan pilih data sales order
                        </DialogDescription>
                    </DialogHeader>
                    <>
                        <Input
                                        ...
                                        <TableRow>
                                            <TableCell>
                                                <button type="button" className="underline text-blue-600"
                                                    onClick={() => {
                                                        const newValue = {
                                                            label: `${item.number} - ${moment(item.date).format('DD/MM/YYYY')} - ${item.customer.name}`,
                                                            value: item.number
                                                        }
