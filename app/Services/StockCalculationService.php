...
class StockCardService
{
    ...
    {
        $balanceIn = ItemTransaction::where([
            ['item_id', $itemId],
            ['transaction_type', 'IN'],
            ['date', '<', $fromDate],
        ])
        ...
        $balanceOut = ItemTransaction::where([
            ['item_id', $itemId],
    ...
    {
        return ItemTransaction::with(['item', 'warehouse', 'linkingItem'])
    ...
    protected function calculateTotalsAndFormatTransactions($itemTransactions, float $startingBalance, Item $item): array
    {
        ...
        foreach ($itemTransactions as $transaction) {
            // Update balance
            $balance += $transaction->transaction_type === 'IN' ? $transaction->quantity : -$transaction->quantity;
